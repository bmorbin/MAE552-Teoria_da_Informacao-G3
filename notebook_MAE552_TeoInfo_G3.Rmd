...
for(clust in 1:num_cluster_init){
  ...
  if(is.null(nrow(gp))) {
    ...
    bounds <- apply(gp,MARGIN = 2, FUN = function(x) quantile(x,c(0.10,0.90))) # definindo os intervalos interquantil para cada indivíduo nesse conjunto de genes
...
```{r}
ind_bound <- max(quantile(unlist(mutualinfo_clust),0.35)[[1]],1); ind_bound # métrica para clusters consistentes
