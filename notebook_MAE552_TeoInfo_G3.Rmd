...
```{r, fig.cap="Bruno Gráfico"}
ggplot(mtcars, aes(x=mpg,y=cyl, color=vs))+geom_point()+labs(title="bruno")+theme(plot.title = element_text(color="red", face = "bold"))
```
```{r}
plot(iris)
```
...
#ver se em cada grupo da pra discriminar tal caracatetiriscva
```
